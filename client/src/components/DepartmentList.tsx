...
function DepartmentList() {
  ...
  const handleEdit = async () => {
    if (!currentDepartment || !formData.Name) return;

    try {
      await departmentAPI.update(currentDepartment.Id, { Name: formData.Name });
      await fetchData();
  ...
  const handleDelete = async () => {
    ...
    try {
      await departmentAPI.delete(currentDepartment.Id);
      await fetchData();
                    ...
                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-500">
                          ...
                          <strong>{currentDepartment?.Name}</strong>? This
                          action cannot be undone and will fail if the
                          department has work units or employees.
                        </p>
                      </div>
                    </div>
                  </div>
